...
import FaylYuklash1 from "./FaylYuklash1"
const Hello = () => {
  ...
  const getData = async () => {
    try {
      ...
      const { data } = await axios.get(
        `${mahalla_url}/categoriya_people_deteile/${id}/`,
        config,
      )
      setData(data)
      console.log(data)
  ...
  const getData2 = async () => {
    try {
      ...
      console.log(config)
      const { data } = await axios.get(
        `${mahalla_url}/create_task_files/${taskId}/`,
        config,
      )
      setFilesData(data)
      console.log(data)
            ...
            <CTableDataCell>
              {item?.people?.map((item) => (
                <div>
                  ...
                  {item?.is_user ? (
                          <CButton size="sm" color="primary">Imzolangan</CButton>
                        ):
                        (<CButton size="sm" color="danger">Imzolanmagan</CButton>)
                        }
                  <br />
                  <br />
                </div>
